...
export default async function Pages({ params }: PageProps) {
  ...
  if (slug.length === 0) {
    const routes = getPageRoutesByLanguage('en')
    const firstPage = routes.find((route) => route.href)
    if (firstPage) {
      redirect(`/en${firstPage.href}`)
    }
  }

  const pathName = slug.join('/')
  const res = await getDocument(pathName, 'en')
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    alternates: {
      canonical: `${Settings.metadataBase}/en/${pathName}`,
    },
  }
}

export function generateStaticParams() {
  const pages = getPageRoutesByLanguage('en')
